<?php echo null; ?>

<footer>
  <div class="container-fluid">
    <footer class="py-5">
      <div class="row text-center">
        <div class="col-3">
          <div class="m-1">
            <img src="https://blackiq.github.io/cdn-university-bookstore/images/new/payment.png" alt="payment">
            <br>
            <br>
            <h6>پرداخت امن از درگاه بانکی</h6>
          </div>
        </div>
        <div class="col-3">
          <div class="m-1">
            <img src="https://blackiq.github.io/cdn-university-bookstore/images/new/Quality.png" alt="Quality">
            <br>
            <br>
            <h6>تضمین کیفیت کالا</h6>
          </div>
        </div>
        <div class="col-3">
          <div class="m-1">
            <img src="https://blackiq.github.io/cdn-university-bookstore/images/new/Guarantee.png" alt="Guarantee">
            <br>
            <br>
            <h6>فرصت 1 روزه بازگشت کالا</h6>
          </div>
        </div>
        <div class="col-3">
          <div class="m-1">
            <img src="https://blackiq.github.io/cdn-university-bookstore/images/new/Support.png" alt="Support">
            <br>
            <br>
            <h6>پشتیبانی 24 ساعته</h6>
          </div>
        </div>
      </div>
      <hr>
      <br>
      <div class="row text-center">
        <div class="col-3">
          <div class="m-1">
            <h5>فهرست عناوین</h5>
            <br>
            <ul class="nav flex-column">
              <li class="nav-item mb-2"><a href="<?php echo $path; ?>/pages/back.php" class="nav-link p-0 text-muted">بازگشت کالا</a></li>
              <li class="nav-item mb-2"><a href="<?php echo $path; ?>/pages/warranty.php" class="nav-link p-0 text-muted"> ضمانت کالا</a></li>
              <li class="nav-item mb-2"><a href="<?php echo $path; ?>/pages/delivery.php" class="nav-link p-0 text-muted">نحوه تحویل کالا</a></li>
            </ul>
          </div>
        </div>
        <div class="col-3">
          <div class="m-1">
            <h5>خدمات مشتریان</h5>
            <br>
            <ul class="nav flex-column">
              <li class="nav-item mb-2"><a href="<?php echo $path; ?>/pages/qustions.php" class="nav-link p-0 text-muted">سئوالات متداول</a></li>
              <li class="nav-item mb-2"><a href="<?php echo $path; ?>/pages/protest.php" class="nav-link p-0 text-muted">ثبت شکایت</a></li>
              <li class="nav-item mb-2"><a href="<?php echo $path; ?>/pages/guide.php" class="nav-link p-0 text-muted">راهنمای خرید</a></li>
            </ul>
          </div>
        </div>
        <div class="col-md-4">
          <div class="m-1">
            <form>
              <h5>با اشتراک در خبرنامه از آخرین و بهترین پیشنهادها مطلع شوید</h5>
              <br>
              <div class="d-flex w-100 gap-2">
                <label for="newsletter1" class="visually-hidden">Email address</label>
                <input id="newsletter1" type="text" class="form-control" placeholder="پست الکترونیکی خودراوارد کنید">
                <button class="btn btn-dark" type="button">اشتراک</button>
              </div>
            </form>
          </div>
        </div>
        <div class="col-md-2">
          <div class="m-1">
            <h5>ارتباط با ما</h5>
            <br>
            <i class="fa fa-twitter"></i>
            &nbsp;
            <i class="fa fa-linkedin"></i>
            &nbsp;
            <i class="fa fa-envelope"></i>
          </div>
        </div>
      </div>
    </footer>
  </div>
  </div>
</footer>